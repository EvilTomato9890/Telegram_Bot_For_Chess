...
from domain.models import Game, GameResult, PlayerStatus, Round, RoundStatus, TournamentStatus
...
class PairingService:
    ...
    def generate_next_round(self, tournament_id: int, actor_id: int, force: bool = False) -> PairingOutcome:
        ...
        active_players = [p for p in self._player_repo.list_all() if p.status == PlayerStatus.ACTIVE]
        ...
        if engine_result.confirmation_request is not None and not force:
            ...
            self._tournament_repo.update_status(
                ...
                pending_pairing_payload=reason,
            )
            ...
            persisted_games.append(
                self._game_repo.add(
                    Game(
                        ...
                        board_number=game.table_number,
        ...
        if engine_result.bye is not None:
            bye_player_id = engine_result.bye.player_id
    ...
    def confirm_next_round(self, tournament_id: int, actor_id: int) -> PairingOutcome:
        """Force next round generation when repeats are unavoidable."""
        ...
        return self.generate_next_round(1, 0, force=True)

    def _require_ongoing_tournament(self):
    ...
    def _build_history(self, active_players: list) -> list[PairingPlayer]:
