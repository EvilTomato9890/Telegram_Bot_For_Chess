...
class ResultService:
    ...
    def submit_player_report(self, telegram_id: int, raw_result: str) -> ReportOutcome:
        ...
        if opp is None:
            return ReportOutcome(game_id=game_id, status="pending", message="Отчет сохранен, ждем соперника.")
        ...
        if opp.reported_result == own.reported_result:
            ...
            return ReportOutcome(game_id=game_id, status="agreed", message="Результат подтвержден.")
